...
function setup(evt) {
    ...
    function chargerEtape5(){
        $("#etape").html("Etape n°4");
        $("#instruction").html("Où souhaitez vous aller ?")
        fade_in($("#destination"));
        $("#etapes").hide();
        selecteur.hide();
        $("#home").hide();
          ...
          //          console.log("marque de la voiture :", marqueVoiture);
            ...
            //        console.log("Niveau actuel :", niveauCarbAct);
                    ...
                    getCapaciteReservoir();
                     niveauAct = capRes*niveauCarbAct; // Calcul le niveau de carburant en litre
                     chargerEtape5();
                     $('#distance_form').submit(function(e){
                        e.preventDefault();
                         calculateDistance();
                        var niveauCarburant = capRes * niveauCarbAct;
        //                console.log("niveau en Litre NEW : ",niveauCarburant);
    ...
    function calculCarburantAct(niveauAct){

        // console.log("niveau Actuel:",niveauAct);
            ...
            if (vehicules[i][0] == marqueVoiture) {
                for (var j = 1; j < vehicules[i].length; j++) {
                    if (marqueVoiture + " " + vehicules[i][j][0] == modeleVoiture) {
                        capRes = vehicules[i][j][1];
        //    console.log("CAPACITE RESERVOIR :",capRes);
                        return capRes;
    ...
    function calculerNbLitres(nvActu, nvSouh, capacite) {
        var nbLitres = nvActu * capacite;
      //  console.log(Math.round(nbLitres));
        return Math.round(nbLitres);
    }
    function calculateDistance(niveauAct) {
    //    console.log("Modele voiture : ",modeleVoiture);
  //      console.log("ratio niveau carburant actuel : ",niveauCarbAct);
    //    console.log("capacité reservoir : ",capRes);
        var niveauCarburant = capRes * niveauCarbAct;
  //      console.log("niveau en Litre : ",niveauCarburant);


       var consomationVoitureParKm = 0.065; // 1km
    ...
    function callback(response, status) {
      ...
      //          console.log(response.rows[0].elements[0].distance);
                ...
                if(distance_in_kilo>0){
                 niveauCarburant = niveauCarbAct * capRes;
                 consoL1km = 0,0625;
    //             console.log("My distance",distance_in_kilo);
                 var Conso = distance_in_kilo * 0.0625;
                 resultat = niveauCarburant - Conso ;
    //             console.log("Res : ",resultat);
    //             console.log("origin:", origin);
                 const mydest= $('#destination').val();
                 const fromplace = $('#origin').val();
                 ...
                 if(resultat>10){
                    ...
                    document.getElementById('distance_form').style.display="";
                    document.getElementById('instruction').style.display="none";
                    document.getElementById("maps").style.display = "inline";
                    document.getElementById("img").style.display="inline";
                    document.getElementById('maps').src ="https://www.google.com/maps/embed/v1/directions?key=AIzaSyBi7TizVTLJIjkU7qWdzR7Z65mrsWYepyA&origin="+fromplace+"+&destination="+mydest+"&avoid=tolls|highways";
                    console.log("Vous pourrez vous rendre à votre destination, il vous restera ",Math.round(resultat)," L");
                    document.getElementById("img").src="resources/v0.9.3/images/pouce-vert.png";
                    $('#Autho').html("Vous pourrez vous rendre à votre destination, il vous restera " + Math.round(resultat)+" L dans votre reservoir");
                    $('#distance_trajet').html("Distance Trajet = "+distance_in_kilo + " km ");

                 }
                 else if(resultat<10 && resultat>0) {
                   console.log(resultat);
                    ...
                    document.getElementById("img").style.display="inline";
                    document.getElementById('maps').src ="https://www.google.com/maps/embed/v1/directions?key=AIzaSyBi7TizVTLJIjkU7qWdzR7Z65mrsWYepyA&origin="+fromplace+"+&destination="+mydest+"&avoid=tolls|highways";
                    console.log("Vous pourrez vous rendre à votre destination, mais il vous restera ",Math.round(resultat)," L dans votre reservoir");
                    document.getElementById("img").src="resources/v0.9.3/images/pouceorange.png";
                    $('#Autho').html("Vous pourrez vous rendre à votre destination, mais il vous restera " + Math.round(resultat)+" L dans votre reservoir");
                    $('#distance_trajet').html("Distance Trajet = "+distance_in_kilo + " km ");
                                 }
                 else if(resultat<0){
                    $('#destination').hide();
       ...
       console.log("Vous ne pourrez pas vous rendre à votre destination, il vous manquera ",Math.round(resultat * -1)," L");
                    document.getElementById("img").src="resources/v0.9.3/images/pouce-rouge.png";
                    $('#Autho').html("Vous ne pourrez pas vous rendre à votre destination, il vous manquera " + Math.round(resultat * -1)+" L dans votre reservoir");
                    $('#distance_trajet').html("Distance Trajet = "+distance_in_kilo + " km ");
        //            console.log("Trajet impossible, il vous manquera ",Math.round(resultat * -1)," L");

                 }


                  function MapDirection(position) {
                      const mydest= $('#destination').val();
                      const fromplace = $('#origin').val();
                      document.getElementById('maps').src ="https://www.google.com/maps/embed/v1/directions?key=AIzaSyBi7TizVTLJIjkU7qWdzR7Z65mrsWYepyA&origin="+fromplace+"+&destination="+mydest+"&avoid=tolls|highways";
                    }
                  ...
                  document.getElementById('maps').src ="https://www.google.com/maps/embed/v1/directions?key=AIzaSyBi7TizVTLJIjkU7qWdzR7Z65mrsWYepyA&origin="+position.coords.latitude+"/"+position.coords.longitude+"+&destination="+mydest+"&avoid=tolls|highways";
...
$("#copy").on("click", (el) => {
  ...
  //Permet d'entrer la valeur Ma position Actuelle dans l'input 'départ' du formulaire
    function maposition(){
              ...
              if (status != google.maps.DistanceMatrixStatus.OK) {
                  ...
                  if (response.rows[0].elements[0].status === "ZERO_RESULTS") {
                      ...
                      if(distance_in_kilo>30){
                        $('#etape').html("RESULTAT");
                        $('#mymap').hide();
                        document.getElementById("img").src="resources/v0.9.3/images/valide.png";
                        document.getElementById("img").style.display="inline";
                        $('#distance_trajet').html("Distance Trajet = "+distance_in_kilo + " km ");
                        $('#Autho').html("Vous pourrez vous rendre à votre destination, il vous restera X litres dans votre réservoir"  + distance_in_kilo + " km ");
                        document.getElementById("form").style.display = "none";
                        document.getElementById('distance_form').style.display="";
                        document.getElementById('instruction').style.display="none";
                        document.getElementById("maps").style.display = "inline";
                       ...
                       function showplace(position) {
                        ...
                        document.querySelector('.myForm input[name = "latitude"]').value = position.coords.latitude;
                        document.querySelector('.myForm input[name = "longitude"]').value = position.coords.longitude;
                        document.getElementById('map').src ="https://www.google.com/maps/embed/v1/directions?key=AIzaSyBi7TizVTLJIjkU7qWdzR7Z65mrsWYepyA&origin="+position.coords.latitude+"/"+position.coords.longitude+"+&destination="+mydest+"&avoid=tolls|highways";
                        ...
                        $('#etape').html("RESULTAT");
                        $('#mymap').hide();
                        document.getElementById("img").src="resources/v0.9.3/images/invalide.jpeg";
                        document.getElementById("img").style.display="inline";
                        $('#distance_trajet').html("Distance Trajet = "+distance_in_kilo + " km ");
                        $('#Autho').html("Vous ne pourrez pas vous rendre à votre destination, il vous manquera X litres dans votre réservoir ");
                        document.getElementById("form").style.display = "none";
                        document.getElementById('distance_form').style.display="";
                        document.getElementById('instruction').style.display="none";
                        document.getElementById("maps").style.display = "inline";
...
// Recupère la localisation de l'utilisateur
function write(){
...
function showPosition(position) {
    ...
    const KEY = "AIzaSyBi7TizVTLJIjkU7qWdzR7Z65mrsWYepyA";
  ...
  fetch(url)
    ...
    console.log(address);
    document.getElementById('from_places').value= address;
    document.getElementById('origin').value= address;
    document.getElementById('from_places').value.style.fontsize='30px';          })
    .catch(err => console.warn(err.message));


      //  document.getElementById('map').src ="https://www.google.com/maps/embed/v1/directions?key=AIzaSyBi7TizVTLJIjkU7qWdzR7Z65mrsWYepyA&origin="+position.coords.latitude+"/"+position.coords.longitude+"&destination=Drancy+France&avoid=tolls|highways";


  }

  function user(locationz){
    getLocation();
    const loc = address;
    console.log("get my location", locationz);
    document.getElementById('from_places').value= address;
    document.getElementById('origin').value= address;
